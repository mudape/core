...
def _records_schema(records):
    ...
    return vol.Schema(
        {vol.Required(CONF_RECORDS): cv.multi_select({name: name for name in records})}
    )
...
class CloudflareConfigFlow(ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_import(
        ...
        if user_input is not None:
            ...
            user_input[CONF_ZONE] = user_input[CONF_ZONE].lower()
            for idx, rec in enumerate(user_input[CONF_RECORDS]):
                record = rec.lower()
                if user_input[CONF_ZONE] not in record:
                    user_input[CONF_RECORDS][idx] = "{record}.{user_input[CONF_ZONE]}"
                else:
                    user_input[CONF_RECORDS][idx] = record
